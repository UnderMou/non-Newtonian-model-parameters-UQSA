...
def process_item(item):
    core_params, exp_data, opt_approach, par_fixed, par_value = item[0], item[1], item[2], item[3], item[4]
    
    XPL_arr = run_identifiability_analysis(core_params, exp_data, opt_approach, par_fixed, par_value)
...
def run_identifiability_analysis(core_params, exp_data, opt_approach, par_fixed, par_value):
    ...
    opt_approach[par_fixed] = [par_value, par_value]
    ...
    for i in range(n_runs):
        ...
        log_message = f"{par_fixed} = {par_value} run: {i+1}|{n_runs}"
        print(log_message)
    ...
    # return par_fixed, par_value, np.mean(obj_func_values)
    # return np.mean(obj_func_values)
    return np.min(obj_func_values)
...
if __name__ == '__main__':
    ...
    for i in range(5):  # 5 parameters
        if i == 4: array = fmcap_spacing(Np, opt_approach)
        else: array = np.linspace(opt_approach['p'+str(i)][0], opt_approach['p'+str(i)][1], Np)

        ranges.append(array.tolist())
    
    with open('post_proc/parameter_identifiability/approach2.1/ranges_approach2.1.csv', 'w', newline='') as csvfile:
    ...
    for i in range(len(ranges)):
        for j in range(len(ranges[i])):
            data.append([core_params, exp_data, opt_approach, 'p'+str(i), ranges[i][j]])

    num_workers = 6

    with multiprocessing.Pool(processes=num_workers) as pool:
    
        res = pool.map(process_item, data)
    ...
    for i in range(N_params):       
        array = res[i*Np:i*Np+Np]
        print(array, "\n")
        results.append(array)
    print(results)
    
    filename = 'post_proc/parameter_identifiability/approach2.1/output_approach2.1.csv'
